...
public class FactionRolesGui extends BaseUIModelScreen<FlowLayout> {
	...
	private void updateList(FlowLayout parent) {
		...
		for(var n : sorted) {
			...
			}).sizing(Sizing.fixed(12), Sizing.fixed(12)).tooltip(Text.translatable("gui.green_resurgence.faction_role_gui.priority_up")));
			l1.child(Components.button(Text.literal("\uD83D\uDF83"), b -> {
			}).sizing(Sizing.fixed(12), Sizing.fixed(12)).tooltip(Text.translatable("gui.green_resurgence.faction_role_gui.priority_down")));
			l.child(l1);
			...
			l.child(Components.button(Text.literal("\uD83D\uDD89"), (v) -> Channels.MAIN.clientHandle().send(new GuildPackets.PermEditRequest(n)))
					.tooltip(Text.translatable("gui.green_resurgence.generic.modify")).sizing(Sizing.fixed(20)));
