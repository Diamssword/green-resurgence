		...
		if(resource != null) {
			...
			if(resource.getType().isItem) {
				if(resource.getType() == UniversalResource.Type.itemtag) {
					...
					var lines = Screen.getTooltipFromItem(client, st);
					lines.add(Text.translatable("gui.green_resurgence.tootltip.ingredient.accept_tag", "#" + resource.getID()).formatted(Formatting.GRAY));
					context.drawTooltip(client.textRenderer, lines, st.getTooltipData(), mouseX, mouseY);
				} else
					context.drawItemTooltip(client.textRenderer, resource.getCurrentItem(time), mouseX, mouseY);

			} else {
				var ls = new ArrayList<>(List.of(TooltipComponent.of(resource.getName(time).asOrderedText())));
				if(resource.getType() == UniversalResource.Type.fluidtag)
					ls.add(TooltipComponent.of(Text.translatable("gui.green_resurgence.tootltip.ingredient.accept_tag", "#" + resource.getID()).formatted(Formatting.GRAY).asOrderedText()));
				context.drawTooltip(client.textRenderer, mouseX, mouseY, ls);
			}
		}
	}
