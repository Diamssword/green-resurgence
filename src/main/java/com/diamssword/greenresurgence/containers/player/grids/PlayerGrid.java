...
public class PlayerGrid extends GridContainer {
    ...
    public int getQuickSlotPriority(ItemStack item) {
        if (this.getName().equals("hotbar")) {
            ...
            if ((eq != null && (eq.getSlotType() == EquipmentSlot.MAINHAND || eq.getSlotType() == EquipmentSlot.OFFHAND)) || item.getItem() instanceof ToolItem)
                return 10;
            else
                return 0;
        } else if (this.getName().equals("player"))
            return 0;
        return 1;
