...
public class CustomPlayerInventory {
    ...
    public void updateItems() {
        this.getAllInventories().forEach(i->{
            var ishot= i == getHotBar();
            var isBags= i == getBags();
            for (int j = 0; j < i.size(); j++) {
                var s=i.getStack(j);

                if(s !=null && !s.isEmpty())
                {
                    if(isBags && s.getItem() instanceof AbstractBackpackItem bi)
                        bi.bagSlotTick(s,parent.getWorld(),parent, j);
                    else
                        s.inventoryTick(parent.getWorld(),parent, j, ishot && this.parent.getInventory().selectedSlot == j);
                }
            }
        });
