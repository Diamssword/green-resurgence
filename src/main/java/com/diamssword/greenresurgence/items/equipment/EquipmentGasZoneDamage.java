...
public class EquipmentGasZoneDamage extends EquipmentToolGas {
	...
	public EquipmentGasZoneDamage(String category, String subCategory, Map<String, EffectLevel> baseEffects, boolean emissive) {
	...
	public float getGasUseOnTarget(LivingEntity user, ItemStack stack, Entity target) {
			...
			for(Entity entity : entities) {
				if(!user.getWorld().isClient) {
					if(attackEntity(user, stack, entity)) {
						used += getGasUseOnTarget(user, stack, entity);
					}
				} else
	...
	public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
		...
		if(remainingUseTicks <= l) {
			...
			if(!user.getWorld().isClient && used > 0) {
				if(!this.tryUseGas(stack, (long) used)) {
