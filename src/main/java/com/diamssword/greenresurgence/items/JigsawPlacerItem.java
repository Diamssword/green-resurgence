...
public class JigsawPlacerItem extends Item implements IStructureProvider {
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if(tag.contains("pos")) {
			...
			if(pos.equals(context.getBlockPos().offset(context.getSide()))) {
				boolean res = loadStructure((ServerPlayerEntity) context.getPlayer(), (ServerWorld) context.getWorld(), structureName, pos, dir);
				tag.remove("pos");
				tag.remove("dir");
				if(!res) {
	...
	public boolean loadStructure(ServerPlayerEntity playerEntity, ServerWorld serverLevel, Identifier structureName, BlockPos blockPos, Direction facing) {
		if(structureName != null) {
			try {
				return loadJigSaw(playerEntity, serverLevel, blockPos, facing);
			} catch(Exception e) {
				return false;
			}
		}
	...
	public boolean loadJigSaw(ServerPlayerEntity playerEntity, ServerWorld world, BlockPos pos, Direction dir) {
