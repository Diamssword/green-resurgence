...
public abstract class AbstractBackpackItem extends Item {
    ...
    public void bagSlotTick(ItemStack stack, PlayerEntity entity, int slot) {

    }

    public boolean bagInventoryTick(ItemStack stack, PlayerEntity player, int slot) {
        if (!player.getWorld().isClient) {
            if (!player.isCreative() && !player.isSpectator() && !isInventoryEmpty(stack)) {
                BackpackEntity.dropItemBackpack(player, stack.copyAndEmpty());
                return false;
            }
        }
        return true;
