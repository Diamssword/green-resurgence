...
public class JigsawHelper {
	public static Optional<Structure.StructurePosition> generate(Structure.Context context, RegistryEntry<StructurePool> structurePool, Optional<Identifier> id, int size, BlockPos pos, boolean useExpansionHack, Optional<Heightmap.Type> projectStartToHeightmap, int maxDistanceFromCenter, BlockRotation rotation) {
		BlockPos blockPos;
		DynamicRegistryManager dynamicRegistryManager = context.dynamicRegistryManager();
		ChunkGenerator chunkGenerator = context.chunkGenerator();
		StructureTemplateManager structureTemplateManager = context.structureTemplateManager();
		HeightLimitView heightLimitView = context.world();
		Random chunkRandom = Random.create();
		...
		if(id.isPresent()) {
			...
			if(optional.isEmpty()) {
				GreenResurgence.LOGGER.error("No starting jigsaw {} found in start pool {}", (Object) identifier, (Object) structurePool.getKey().map(key -> key.getValue().toString()).orElse("<unregistered>"));
				return Optional.empty();
			}
	...
	static final class StructurePoolGenerator {
		...
		void generatePiece(PoolStructurePiece piece, MutableObject<VoxelShape> pieceShape, int minY, boolean modifyBoundingBox, HeightLimitView world, NoiseConfig noiseConfig) {
			...
			block0:
			for(StructureTemplate.StructureBlockInfo structureBlockInfo : structurePoolElement.getStructureBlockInfos(this.structureTemplateManager, blockPos, blockRotation, this.random)) {
				...
				if(optional.isEmpty()) {
					GreenResurgence.LOGGER.warn("Empty or non-existent pool: {}", (Object) registryKey.getValue());
					continue;
				}
				...
				if(registryEntry2.value().getElementCount() == 0 && !registryEntry2.matchesKey(StructurePools.EMPTY)) {
					GreenResurgence.LOGGER.warn("Empty or non-existent fallback pool: {}", (Object) registryEntry2.getKey().map(key -> key.getValue().toString()).orElse("<unregistered>"));
					continue;
				}
				...
				while(iterator.hasNext() && (structurePoolElement2 = (StructurePoolElement) iterator.next()) != EmptyPoolElement.INSTANCE) {
					...
					for(BlockRotation blockRotation2 : BlockRotation.randomRotationOrder(this.random)) {
						...
						int l = !modifyBoundingBox || blockBox2.getBlockCountY() > 16 ? 0 : list2.stream().mapToInt(blockInfo -> {
							...
							RegistryKey<StructurePool> registryKey1 = StructurePoolGenerator.getPoolKey(blockInfo);
							Optional<RegistryEntry.Reference<StructurePool>> optional1 = this.registry.getEntry(registryKey1);
							Optional<RegistryEntry> optional2 = optional1.map(entry -> ((StructurePool) entry.value()).getFallback());
							int i1 = optional1.map(entry -> ((StructurePool) entry.value()).getHighestY(this.structureTemplateManager)).orElse(0);
							int j1 = optional2.map(entry -> ((StructurePool) entry.value()).getHighestY(this.structureTemplateManager)).orElse(0);
