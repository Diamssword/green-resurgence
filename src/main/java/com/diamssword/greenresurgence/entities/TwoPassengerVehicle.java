...
public class TwoPassengerVehicle extends Entity  implements GeoEntity {
    ...
    public void tick() {
        ...
        if (this.isLogicalSideForUpdatingMovement()) {
            Vec3d velocity = this.getVelocity(); // Get entity velocity
            float yawRad = (float) Math.toRadians(-this.getYaw()); // Convert yaw to radians (negative because of MC's coordinate system)

            // Compute the entity's forward unit vector from yaw
            Vec3d forward = new Vec3d(-Math.sin(yawRad), 0, Math.cos(yawRad));
            var curr=velocity.dotProduct(forward);
            //System.out.println(lastSpeed);
            if(lastSpeed>0.6 && curr<0.01f && getWorld().isClient)
            {
                Channels.MAIN.clientHandle().send(new EntitiesPackets.OnWallCollided(lastSpeed));
    ...
    private void updateVelocity() {
        ...
        if (this.lastLocation == BoatEntity.Location.IN_AIR && this.location != BoatEntity.Location.IN_AIR && this.location != BoatEntity.Location.ON_LAND) {
            ...
            if (this.location == BoatEntity.Location.IN_WATER) {
                ...
                this.velocityDecay =this.pressingForward?1f:Math.min(0.90f,this.nearbySlipperiness*1.5f);
            } else if (this.location == BoatEntity.Location.ON_LAND) {
                this.velocityDecay =this.pressingForward?1f:Math.min(0.90f,this.nearbySlipperiness*1.5f);//Math.min(0.99f,this.nearbySlipperiness*1.5f);
            ...
            var bound=2.5f+Math.min(heightSpeedCap,30f);
            this.setVelocity(MathHelper.clamp(vec3d.x * (double) this.velocityDecay,-bound,bound), vec3d.y + e, MathHelper.clamp(vec3d.z * (double) this.velocityDecay,-bound,bound));
            this.yawVelocity = (this.yawVelocity * this.velocityDecay)/2f;
            if (f > 0.0) {
