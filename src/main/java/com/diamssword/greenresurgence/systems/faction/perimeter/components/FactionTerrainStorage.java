...
public class FactionTerrainStorage implements NamedScreenHandlerFactory, Inventory {
	...
	public void addIfMissing(BlockPos pos, Inventory inventory) {
		if(!inventoriesPos.contains(pos)) {
			...
			if(inventory instanceof GenericStorageBlockEntity in1) {in1.addListener(l -> formated.refresh());}
		}
	}
	...
	public static class ScreenHandler extends MultiInvScreenHandler {
		...
		public void updateSlotStacks(int revision, List<ItemStack> stacks, ItemStack cursorStack) {
			...
			if(bl && handler != null) {handler.run();}
		}
		...
		protected Slot createSlot(IGridContainer container, int index, int x, int y) {
			if(container.getInventory() instanceof FormattedInventory) {return new FormattedInventory.MySlot((FormattedInventory) container.getInventory(), index, x, y);} else {
		...
		public ItemStack quickMove(PlayerEntity player, int invSlot) {
			...
			if(slot != null && slot.hasStack()) {
				...
				if(cont != null) {
					...
					if(!originalStack.isEmpty()) {slot.insertStack(originalStack);}
				}
			}
			return ItemStack.EMPTY;
		}
		...
		protected boolean insertItem(IGridContainer oriign, ItemStack stack, boolean fromContainer) {
			if(fromContainer) {return super.insertItem(oriign, stack, true);}
		...
		public boolean customSlotCLick(int slotIndex, int button, SlotActionType actionType, PlayerEntity player) {
			...
			if(actionType == SlotActionType.PICKUP) {
				...
				var slot3 = this.slots.get(slotIndex);
		...
		public void onSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity player) {
			...
			try {
				if(!customSlotCLick(slotIndex, button, actionType, player)) {super.onSlotClick(slotIndex, button, actionType, player);}
