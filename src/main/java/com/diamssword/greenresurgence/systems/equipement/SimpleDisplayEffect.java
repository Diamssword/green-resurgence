...
public class SimpleDisplayEffect implements IEquipmentEffect {
	...
	public void addTooltips(UpgradeActionContext ctx, AdvEquipmentSlot slot, List<Text> tooltip) {
		...
		if(lvl > 0) {
			tooltip.add(Text.translatable("attribute.modifier." + (ctx.context == UpgradeActionContext.ItemContext.UPGRADE ? "plus" : "equals") + ".0", MODIFIER_FORMAT.format(lvl), Text.translatable("equipment." + GreenResurgence.ID + ".upgrade.tooltip." + id)).formatted(Formatting.BLUE));
		}
	}
}
