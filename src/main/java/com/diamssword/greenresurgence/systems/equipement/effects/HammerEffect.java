...
public class HammerEffect implements IEquipmentEffect {
	@Override
	...
	public List<TagKey<Item>> getTags(UpgradeActionContext ctx) {
		return List.of(Lootables.HAMMER);
	}
	...
	public void addTooltips(UpgradeActionContext ctx, AdvEquipmentSlot slot, List<Text> tooltip) {
		if(slot == AdvEquipmentSlot.MAINHAND) {
			tooltip.add(Text.translatable("equipment." + GreenResurgence.ID + ".upgrade.tool.hammer").formatted(Formatting.BLUE));
		}
	}
}
