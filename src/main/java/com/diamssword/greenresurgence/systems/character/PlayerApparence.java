...
import com.diamssword.greenresurgence.http.ApiSkinValues;
...
public class PlayerApparence {
	...
	public void refreshSkinData() {
		var car = parent.player.getComponent(Components.PLAYER_CHARACTERS).getCurrentCharacter();
		if (car != null) {
	...
	public void refreshSkinDataForFakePlayer(PlayerEntity truePlayer) {
		var car = truePlayer.getComponent(Components.PLAYER_CHARACTERS).getCurrentCharacter();
		if (car != null) {
			skinDatas = car.appearence;
			if (skinDatas != null) {
				scaledHeight = ApiSkinValues.HeightMToMCScale(1, skinDatas.size);
	...
	public Optional<ClothData> getClothDatas(ClothingLoader.Layer layer) {
		if (layer == ClothingLoader.Layer.hair) {
			...
			return Optional.empty();
		} else if (layer == ClothingLoader.Layer.underwear && cloths.get(layer) == null) {
			if (skinDatas != null)
				return Optional.of(new ClothData(skinDatas.underwear, false, -1));
			else
				return Optional.of(new ClothData("1", false, -1));
		}
