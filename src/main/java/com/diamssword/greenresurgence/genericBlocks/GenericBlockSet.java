...
public class GenericBlockSet {
	...
	public void modelGenerator(ItemModelGenerator generator) {
		...
		itemGroups.forEach((k, v) -> {
			var f = generatedBlocks.stream().filter(v1 -> k.equals(v1.props.itemGroup)).findFirst();
			f.ifPresent(generatedBlockInstance -> generator.register(Registries.ITEM.get(new Identifier(GreenResurgence.ID, this.subdomain + "_" + k)), new Model(Optional.of(new Identifier(GreenResurgence.ID, "item/" + this.subdomain + "_" + generatedBlockInstance.name)), Optional.empty())));
		});
	}
		...
		generatedBlocks.forEach(b -> {
			if(b.props.genBlockState) {
				...
				switch(b.props.type) {
					...
					case PILLAR -> {
						if(b.props.toggleable) {
							...
							generator.blockStateCollector.accept(helper.createVariantsStates(VariantsBlockStateSupplier.create(b1,
									BlockStateVariant.create().put(VariantSettings.MODEL, helper.getBlockModelId(b.name, b.props.modelPath))).coordinate(
									ModelHelper.fillMultistateVariantMap(BlockStateVariantMap.create(Properties.HORIZONTAL_FACING, b1.getToggleProp()),
					...
					case CONNECTED_PILLAR -> {
						...
						generator.blockStateCollector.accept(helper.createVariantsStates(VariantsBlockStateSupplier.create(b.block,
										...
										ModelHelper.fillConnectedBlockVariantMap(BlockStateVariantMap.create(Properties.HORIZONTAL_FACING, SideShelfBlock.MODEL),
												helper.getBlockModelId(b.name, b.props.modelPath), helper.getBlockModelId(b.name, b.props.modelPath).withSuffixedPath("_middle"), helper.getBlockModelId(b.name, b.props.modelPath).withSuffixedPath("_left"), helper.getBlockModelId(b.name, b.props.modelPath).withSuffixedPath("_right")))
								, variants));
					...
					case TABLE -> {
						generator.blockStateCollector.accept(helper.alterSchematic(b.block, b.props.model == ModelType.TABLE_DECORLESS ? "table_decorless" : "table", "id", b.name));
					}
					case PANE -> {
	...
	protected static class GenericBlockRegisterInstance {
		...
		public GenericBlockRegisterInstance customItemModelPath(Function<String, Identifier> factory) {
			blocks.values().forEach(v -> {
		...
		public GenericBlockRegisterInstance schematicBlockStateSupplier(BiFunction<Block, String, SchematicBlockStateSupplier> factory) {
			blocks.values().forEach(v -> {
	...
	public static class GenericBlockProp {
		...
		protected Function<String, Identifier> itemModelPath;
		protected BiFunction<Block, String, SchematicBlockStateSupplier> blockstateSupplier;
	...
	public enum HitBox {
		...
		TABLE(Block.createCuboidShape(0, 12, 0, 16, 16, 16), false),

		CARPET(Block.createCuboidShape(0, 0, 0, 16, 1, 16)),
		SMALL_SIDE_CARPET(Block.createCuboidShape(0, 0, 0, 16, 2, 2), true, true),
		CORNER(Block.createCuboidShape(0, 0, 8, 16, 8, 16), true, true),
		CORNER_UP(Block.createCuboidShape(0, 8, 8, 16, 16, 16), true, true),
		...
		SMALL_WALL(Block.createCuboidShape(0, 0, 6, 16, 10, 10), true, true),
		WALL(Block.createCuboidShape(0, 0, 6, 16, 14, 10), true, true),
		SMALL_WALL_NEGATIVE(Block.createCuboidShape(0, -6, 6, 16, 10, 10), true, true);
