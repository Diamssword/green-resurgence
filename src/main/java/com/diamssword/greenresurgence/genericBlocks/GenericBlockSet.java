...
public class GenericBlockSet {
        ...
        this.blocks.forEach(entry -> {
            entry.blocks.values().forEach(v -> {
                ...
                for (String name : entry.names) {
                    ...
                    switch (v.type) {
                        case SIMPLE ->
                                genericRegisterHelper(name, v, v.toggleable ? new GenericBlockToggleable(settings, v) : new GenericBlock(settings, v));
                        case PILLAR ->
                                genericRegisterHelper(name, v, v.toggleable ? new GenericPillarToggleable(settings, v) : new GenericPillar(settings, v));
                        case CONNECTED_PILLAR ->
                                genericRegisterHelper(name, v, new GenericConnectedPillar(settings, v));
                        case OMNI_BLOCK ->
                                genericRegisterHelper(name, v, v.toggleable ? new OmniBlockToggleable(settings, v) : new OmniBlock(settings, v));
                        ...
                        case STAIRS ->
                                genericRegisterHelper(name, v, new StairsBlock(Blocks.STONE.getDefaultState(), settings));
                        case LECTERN ->
    ...
    public void modelGenerator(ItemModelGenerator generator) {
        ...
        itemGroups.forEach((k, v) -> {
            var f = generatedBlocks.stream().filter(v1 -> k.equals(v1.props.itemGroup)).findFirst();
            f.ifPresent(generatedBlockInstance -> generator.register(Registries.ITEM.get(new Identifier(GreenResurgence.ID, this.subdomain + "_" + k)), new Model(Optional.of(helper.getBlockModelId(generatedBlockInstance.name, generatedBlockInstance.props.modelPath)), Optional.empty())));
        });
    }
