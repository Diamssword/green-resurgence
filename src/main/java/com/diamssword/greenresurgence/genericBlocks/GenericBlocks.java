...
public class GenericBlocks {
	...
	private static final float HIGH_CHAIR = 0.1f;
	...
	static {
		...
		//wool
		teien.create(simpleList("", "_wool", "brown", "gray", "red", "cyan", "green", "yellow", "lime", "blue", "light_gray")).addSub(BlockType.SIMPLE).addSub(BlockType.OMNI_BLOCK, ModelType.CARPET, SubBlock.CARPET).addSub(BlockType.ALL_SIDE_PANEL, ModelType.CARPET, SubBlock.WALL).setTransparency(Transparency.OPAQUE).addGroup("wool").sound(BlockSoundGroup.WOOL);
		//door
	...
	static {
		...
		diams.create("road_tire_spike_cross", "road_tire_spike").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.CARPET_FIXED).setTransparency(Transparency.TRANSPARENT).disableGen(true).setTransparency(Transparency.NOTFULL).setDamage(1f).notSolid();
		...
		diams.create("lamp_street").addSub(BlockType.PILLAR, ModelType.CARPET, HitBox.FULL).togglable().light(12).setTransparency(Transparency.CUTOUT).disableGen(true);
		...
		//cartons
		diams.create(simpleList("carton_petit", "", "", "_travers", "_ouvert", "_entreouvert")).addGroup("cartons_petit").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.MEDIUM).setTransparency(Transparency.NOTFULL).disableGen(true);
		//register
		...
		//table nap
		diams.create(simpleList("table_square_nap_", new String[]{"blue_", "green_", "red_"}, new String[]{"center", "border", "corner", "end", "middle", "solo"})).addSub(BlockType.PILLAR).setTransparency(Transparency.CUTOUT).addModelPath("table/table_square_nap/").addGroup("table_square_nap").disableGen(true);
		//chair wool
		...
		//chair slab
		diams.create("tire_car_flat", "stool_ottoman_resort", "stool_waterhyacinth").addSub(BlockType.PILLAR, ModelType.SLAB, HitBox.FIXED_SLAB).disableGen(true).seat(CHAIR_SLAB).setTransparency(Transparency.NOTFULL);
		//chair cutout
		...
		diams.create("paint_bucket_blue_red").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.CARPET_FIXED).setTransparency(Transparency.CUTOUT).disableGen(true);
		...
		diams.create("electrical_power_box").addSub(BlockType.OMNI_BLOCK, ModelType.SLAB, HitBox.CARPET).togglable().setTransparency(Transparency.CUTOUT).disableGen(true).addGroup("electrical");
		diams.create("electrical_power_box_broken").addSub(BlockType.OMNI_BLOCK, ModelType.SLAB, HitBox.CARPET).setTransparency(Transparency.CUTOUT).disableGen(true).addGroup("electrical");
		diams.create("electrical_power_box_broken_door").addSub(BlockType.PILLAR, ModelType.SLAB, HitBox.CARPET_FIXED).setTransparency(Transparency.CUTOUT).disableGen(true).addGroup("electrical");
		diams.create("generator_small").addSub(BlockType.PILLAR).setTransparency(Transparency.CUTOUT).disableGen(true).addGroup("electrical");

		diams.create(simpleList("vine", "", "_top", "_small", "_large", "")).addSub(BlockType.OMNI_BLOCK, ModelType.SIMPLE, HitBox.CARPET).notSolid().setTransparency(Transparency.CUTOUT).disableGen(true).addGroup("vines");
		//umbrela
		...
		diams.create("cutting_board").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.CARPET_FIXED).setTransparency(Transparency.CUTOUT).disableGen(true);
		//ladders
		...
		diams.create("glass_trapdoor", "broken_glass_trapdoor").addSub(BlockType.TRAPDOOR).setTransparency(Transparency.TRANSPARENT).disableGen(false);
		//fences
		...
		diams.create("air_conditioner_right", "air_conditioner_left").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.SLAB).setTransparency(Transparency.CUTOUT).disableGen(true);

		diams.create("lamp/traffic_lamp_horizontal").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.FIXED_SLAB_TOP).multistate(4).light(10).setTransparency(Transparency.NOTFULL).addGroup("signs").disableGen(true);
		diams.create("lamp/traffic_lamp_vertical").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.SLAB).multistate(4).light(10).setTransparency(Transparency.NOTFULL).addGroup("signs").disableGen(true);
		diams.create(simpleList("sign_", "_post", "stop", "parking", "no_way", "exclamation", "arrow_right", "arrow_left")).addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.CARPET).disableGen(true).addGroup("signs");
		diams.create(simpleList("sign_", "_carpet", "stop", "parking", "no_way", "exclamation", "arrow_right", "arrow_left")).addSub(BlockType.OMNI_BLOCK, ModelType.CARPET, HitBox.CARPET).disableGen(true).addGroup("signs");
		diams.create(append(simpleList("concrete_road_side_drainage_channel_", "", "angle", "angle_sidewalk", "grid", "grid_sidewalk", "straight", "straight_sidewalk"), "street_drain")).addSub(BlockType.PILLAR).disableGen(true).addGroup("sidewalk");
		diams.create(simpleList("concrete_road_side_drainage_channel_", "_half", "angle", "grid", "straight")).addSub(BlockType.PILLAR, ModelType.SLAB, HitBox.FIXED_SLAB).disableGen(true).addGroup("sidewalk");
		diams.create("concrete_road_side_drainage_channel_grid_half_sidewalk", "concrete_road_side_drainage_channel_straight_half_sidewalk").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.STAIR).disableGen(true).addGroup("sidewalk");
		diams.create("concrete_road_side_drainage_channel_angle_half_sidewalk_outer").addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.FIXED_SLAB).disableGen(true).addGroup("sidewalk");
		diams.create("concrete_road_side_drainage_channel_angle_half_sidewalk_inner").addSub(BlockType.PILLAR).setTransparency(Transparency.CUTOUT).disableGen(true).addGroup("sidewalk");

		diams.create(simpleList("kitchen_cabinet_", new String[]{"square_", "sink_", "glass_", "drawer_", "door_", "classic_"}, allColors(""))).addSub(BlockType.PILLAR).disableGen(true).addGroup("kitchen_cabinet");
		...
		diams.create(append(simpleList("table/folding_table/table_folding_", "", "spruce", "oak"), "small_wood_table", "table/cafe/table_cafe_oak", "table/cafe/table_cafe_spruce")).addSub(BlockType.PILLAR).setTransparency(Transparency.NOTFULL).addGroup("tables").disableGen(true);
		diams.create(simpleList("table/", "", "spruce_fancy", "oak_fancy")).addSub(BlockType.TABLE, ModelType.SIMPLE, HitBox.TABLE).setTransparency(Transparency.NOTFULL).disableGen(true).addGroup("tables");
		diams.create(simpleList("table/", "", "basic_oak", "basic_spruce")).addSub(BlockType.TABLE, ModelType.TABLE_DECORLESS, HitBox.TABLE).setTransparency(Transparency.NOTFULL).disableGen(true).addGroup("tables");
		diams.create(allColors("table/plastic_old_")).addSub(BlockType.TABLE, ModelType.TABLE_DECORLESS, HitBox.TABLE).setTransparency(Transparency.NOTFULL).customItemModelPath(n -> {
			var i = n.contains("light_") ? n.indexOf("light_") : n.lastIndexOf("_") + 1;
			return GreenResurgence.asRessource("block/diams/table/plastic_old/table_plastic_old_full_" + n.substring(i));
		}).schematicBlockStateSupplier((b, n) -> {
			var m = new HashMap<String, String>();
			var i = n.contains("light_") ? n.indexOf("light_") : n.lastIndexOf("_") + 1;
			m.put("id", "table/plastic_old");
			m.put("sub", "table_plastic_old_");
			m.put("color", n.substring(i));

			return new SchematicBlockStateSupplier(b, "table_color", m);
		}).disableGen(true).addGroup("tables");

		//cloths
		diams.create(simpleList("hanging_cloth_", "", "short_green", "short_brown", "short_blue", "shirt_red", "shirt_purple", "shirt_gray", "empty", "dress_green", "dress_brown", "dress_blue")).addSub(BlockType.PILLAR, ModelType.PILLAR, HitBox.SMALL_WALL_NEGATIVE).disableGen(true).addGroup("cloths").setTransparency(Transparency.TRANSPARENT);
		diams.create("hanging_cloth_wood").addSub(BlockType.CONNECTED_PILLAR, ModelType.PILLAR, HitBox.FIXED_SLAB).setTransparency(Transparency.NOTFULL).addGroup("cloths").disableGen(true);

		//vent
		diams.create("vent_white_metal_down").addSub(BlockType.PILLAR, ModelType.SIMPLE, HitBox.CORNER).disableGen(true).addGroup("vent");
		diams.create("vent_white_metal_up").addSub(BlockType.PILLAR, ModelType.SIMPLE, HitBox.CORNER_UP).disableGen(true).addGroup("vent");
